...
By default, pip accesses `https://pypi.org/simple/` to get data about third-party packages. But for users in China, the speed of visiting this website is often not very good, so users in China can use mirrors provided by Douban or Tsinghua University instead of the default download source, and then install third-party packages as usual.
...
There are many websites in China that provide network API services, such as Juhe Data and Baidu AI Cloud. These sites provide both free and paid data interfaces. Outside China, the site [{API}Search](http://apis.io/) also provides similar services. Interested readers can study them by themselves. The example below shows how to use the third-party library [`requests`](http://docs.python-requests.org/zh_CN/latest/), a third-party library for accessing network resources through HTTP, to access a network API, get domestic news, and display the news title and link. In this example, we use the domestic-news data interface provided by [Tian API](https://www.tianapi.com/). Its identity credential, the API key, needs to be requested by registering on the website. After registering an account on the Tian API website, an API key will be assigned automatically, but to access the interface and get data, you still need to bind and verify your email or mobile phone, and then apply for the interface you want to use.

<img src="res/20210820151134.png" alt="image-20210820151134034" width="100%">

For Python to access the network through a URL, we recommend the third-party library `requests`. It is simple and powerful, but it needs to be installed by yourself.
